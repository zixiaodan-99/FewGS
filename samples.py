...
def sample_datasets(data, dataset, task, n_way, m_support, n_query):
    ...
    # print("======samples18 before==========")
    # print(support_list)
    random.shuffle(support_list)
    # print("======samples21 after==========")
    # print(support_list)
    l = [i for i in range(0, len(data)) if i not in support_list]
    query_list = random.sample(l, n_query)
    # print(query_list.__len__())
    # [358, 789, 1005, 873, 11, 735, 1353, 654, 1114, 713, 590, 802, 1229, 1192, 1220, 890, 670, 929, 468, 793, 1203, 576, 433, 705, 291, 181, 1020, 513, 439, 600, 1370, 869, 391, 363, 945, 1176, 120, 555, 3, 1327, 322, 304, 1296, 881, 94, 551, 36, 1197, 330, 1323, 943, 618, 768, 190, 142, 752, 610, 348, 12, 199, 629, 993, 366, 560, 573, 1188, 1390, 603, 1059, 376, 837, 1143, 1187, 460, 1218, 1027, 354, 1398, 567, 241, 927, 254, 812, 1313, 46, 1075, 243, 422, 60, 586, 1208, 1235, 338, 1392, 708, 286, 1077, 635, 998, 785, 1269, 1288, 214, 113, 1164, 81, 386, 108, 61, 315, 796, 736, 1248, 520, 318, 473, 180, 1266, 1360, 783, 298, 880, 720, 864, 959, 1333, 792, 1224]
    support_dataset = data[torch.tensor(support_list)]
    # print(support_dataset)  # MoleculeDataset(10)
    query_dataset = data[torch.tensor(query_list)]
    # print(query_dataset)     # MoleculeDataset(128)
    # print("======next=======")

    return support_dataset, query_dataset, support_list, query_list
