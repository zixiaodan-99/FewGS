...
def main(dataset, input_model_file, gnn_type, add_similarity, add_selfsupervise, add_masking, add_weight, m_support):
    ...
    parser.add_argument('--meta_lr', type=float, default=0.001) #可调参数
    parser.add_argument('--update_lr', type=float, default=0.4) #0.4 #可调参数
    ...
    parser.add_argument('--add_weight', type=float, default=0.1) #可调参数
    ...
    # model.to(device)
    # model.load_state_dict(torch.load('mymodel/model.pth'))
    print(args.dataset)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch % 1 == 0:
            ...
            fw.write("\n")
            fw.close()
        if epoch % 1 == 0:#torch.save保存模型参数，.state_dict获取权重和偏差
            torch.save(model.state_dict(), 'mymodel/model.pth')
